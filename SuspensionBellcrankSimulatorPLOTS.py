...
print("\033[2;30;41m Upright Check:\033[0;0m The magnitude of all the elements in upright vectors should be constant. We can "+ \
          "verify this by checking that the values in upright_vector_mag are all equal at a high level, so this should be true: "+ \
                  '\033[4m'+str(np.all(np.round(suspension_geometry_dict_front["upright_vector_mag"],4) == \
                                       np.round(suspension_geometry_dict_front["upright_vector_mag"][0],4)))+'\033[0m')

print("\n\033[2;30;41m Pushrod Check:\033[0;0m The magnitude of all the elements in pushrod vectors should be constant. We can "+ \
          "verify this by checking that the norms of the vectors defining the pushrod are all equal at a high level, so this should be true: "+ \
                  '\033[4m'+str(np.all(pushrod == pushrod[0]))+'\033[0m')
    
print("\n\033[2;30;41m Steering Rod Check:\033[0;0m The magnitude of all the elements in steering rod vectors should be constant. We can "+ \
          "verify this by checking that the norms of the vectors defining the steering rod while the assembly is compressing and steering "+ \
              "are all equal at a high level, so this should be true: "+ \
                  '\033[4m'+str(np.all(steeringrod_steer == steeringrod_compress))+'\033[0m')

print("\n\033[2;30;41m Steering Arm Check:\033[0;0m The magnitude of all the elements in steering arm (moment arm applied to the upright by the steering rod) "+ \
      "vectors should be constant. We can "+ \
          "verify this by checking that the norms of the vectors defining the steering arm while the assembly is compressing and steering "+ \
              "are all equal at a high level, so this should be true: "+ \
                  '\033[4m'+str(np.all(steeringarm_steer == steeringarm_compress))+'\033[0m')

print('\n\033[2;30;41m Shock Absorber Forces Check:\033[0;0m The Tangent and Normal components of the forces exerted on the bellcrank from normal force '+ \
      'by the shock absorber should sum to form the force vectors defining the shock absorber, the magnitudes of which should be equal (Note that the vectors '+ \
          ...
          'Magnitudes: \033[4m'+str(np.all(np.round((\
             (suspension_forces_dict_front["s_t_f_m"])**2 + \
                 (suspension_forces_dict_front["s_n_f_m"])**2)**(1/2),4) == suspension_forces_dict_front["f_m_s"]))+'\033[0m')
    
print('\n\033[2;30;41m Pushrod Forces Check:\033[0;0m The Tangent, Normal, and Binormal components of the forces exerted on the bellcrank from normal force '+ \
      'by the suspension rod should sum to form the force vectors defining the suspension rod, the magnitudes of which should be equal (Note that the vectors '+ \
          ...
          'Magnitudes: \033[4m'+str(np.all(np.round(((suspension_forces_dict_front["r_i_p_f_m"])**2 + \
             (suspension_forces_dict_front["r_t_f_m"])**2 + \
                 (suspension_forces_dict_front["r_n_f_m"])**2)**(1/2),4) == np.round(np.abs(suspension_forces_dict_front["u_d_f_m"]),4)))+'\033[0m')
    
if not np.all(np.round(suspension_geometry_dict_front["upright_vector_mag"],4) == \
                     np.round(suspension_geometry_dict_front["upright_vector_mag"][0],4)) \
    or not np.all(pushrod == pushrod[0]) \
    or not np.all(steeringrod_steer == steeringrod_compress) \
    or not np.all(steeringarm_steer == steeringarm_compress) \
    or not np.all(np.round(( \
       (suspension_forces_dict_front["s_t_f_m"])**2 + \
           (suspension_forces_dict_front["s_n_f_m"])**2)**(1/2),4) == suspension_forces_dict_front["f_m_s"]) \
    or not np.all(np.round(((suspension_forces_dict_front["r_i_p_f_m"])**2 + \
       (suspension_forces_dict_front["r_t_f_m"])**2 + \
...
print("\033[2;30;41m Upright Check:\033[0;0m The magnitude of all the elements in upright vectors should be constant. We can "+ \
          "verify this by checking that the values in upright_vector_mag are all equal at a high level, so this should be true: "+ \
                  '\033[4m'+str(np.all(np.round(suspension_geometry_dict_rear["upright_vector_mag"],4) == \
                                       np.round(suspension_geometry_dict_rear["upright_vector_mag"][0],4)))+'\033[0m')
      ...
      'by the shock absorber should sum to form the force vectors defining the shock absorber, the magnitudes of which should be equal (Note that the vectors '+ \
          ...
          'Magnitudes: \033[4m'+str(np.all(np.round(( \
             (suspension_forces_dict_rear["s_t_f_m"])**2 + \
                 (suspension_forces_dict_rear["s_n_f_m"])**2)**(1/2),4) == suspension_forces_dict_rear["f_m_s"]))+'\033[0m')

print('\n\033[2;30;41m Pushrod Forces Check:\033[0;0m The Tangent, Normal, and Binormal components of the forces exerted on the bellcrank from normal force '+ \
      'by the suspension rod should sum to form the force vectors defining the suspension rod, the magnitudes of which should be equal (Note that the vectors '+ \
          'will not be tested due to symmetry issues with the components) - '+ \
          'Magnitudes: \033[4m'+str(np.all(np.round(((suspension_forces_dict_rear["r_i_p_f_m"])**2 + \
             (suspension_forces_dict_rear["r_t_f_m"])**2 + \
                 (suspension_forces_dict_rear["r_n_f_m"])**2)**(1/2),4) == np.round(np.abs(suspension_forces_dict_rear["u_d_f_m"]),4)))+'\033[0m')
    
if not np.all(np.round(suspension_geometry_dict_rear["upright_vector_mag"],4) == \
    ...
    or not np.all(np.round(((suspension_forces_dict_rear["r_i_p_f_m"])**2 + \
       (suspension_forces_dict_rear["r_t_f_m"])**2 + \
...
fig_manual.write_html("Animation of Dyanamics Manually (via sliders).html", auto_open=True)
